#!/usr/bin/env bash
#=====================================================================================
# n8n SECURE BOOTSTRAP (Ubuntu 22.04/24.04) — one‑shot, idempotent installer (v1.2)
#-------------------------------------------------------------------------------------
# Hardened, resumable installer for running n8n + PostgreSQL behind Caddy (auto‑HTTPS)
...
#   • Basic Auth in front of n8n (Caddy `basic_auth`, creds embedded)
#   • Optional CrowdSec IPS
#
# Resumable & safe:
#   • Each step checkpointed in /var/lib/n8n-bootstrap/steps
#   • Can be re-run anytime; only missing steps execute
#   • SSH migration keeps port 22 until you explicitly allow closing it
#   • NEW: Before any reboot, the script asks you to confirm you can SSH in
#          on the configured port; only then it proceeds to reboot.
#
# Quick start
#   1) Copy this file to the server, edit CONFIG below (set DOMAIN/EMAIL, user/key).
#   2) Run as root:  bash ./n8n-secure-bootstrap.sh
#   3) If it reboots, run the same command again; it resumes automatically.
#
# After install
#   • Visit: https://<DOMAIN> — you’ll see Basic Auth first (admin/<printed-or-filed-pass>),
#     then n8n Owner setup screen. SMTP for n8n is optional but recommended.
#
#=====================================================================================
set -Eeuo pipefail
IFS=$'\n\t'
...
CREATE_NEW_USER=true               # true|false — recommended
NEW_USER="deploy"
...
CLOSE_PORT_22_AFTER_MIGRATION=false  # set true to remove port 22 after migration completes

# Reboot behaviour
CONFIRM_BEFORE_REBOOT=true         # ask to confirm SSH works BEFORE rebooting (recommended)
AUTO_REBOOT=false                  # if true, skip confirmation (useful for CI)

# n8n settings
N8N_VERSION="latest"              # or pin e.g. "1.64.0"
TZ="Etc/UTC"
N8N_DATA_DIR="/opt/n8n"

# n8n security / auth (front door)
BASIC_AUTH_ENABLE=true             # protect n8n with Basic Auth at the proxy
BASIC_AUTH_USER="admin"
BASIC_AUTH_PASSWORD="changeMe"    # if left default/empty, a strong random will be generated
BASIC_AUTH_SECRETS_FILE="/root/n8n_basic_auth.txt"  # generated creds saved here (600)
...
abort_if_placeholder_domain() {
  if [[ "$DOMAIN" == "example.com" || -z "$DOMAIN" ]]; then
    fail "DOMAIN is not set. Edit CONFIG at the top of this script."; exit 1
  fi
  if [[ "$EMAIL" == "admin@example.com" || -z "$EMAIL" ]]; then
    fail "EMAIL is not set. Edit CONFIG at the top of this script."; exit 1
  fi
}
...
step_00_preflight() {
  ...
  info "Starting n8n secure bootstrap v$VERSION_TAG — log: $LOG_FILE"
  bootstrap_packages
...
step_40_ssh() {
  ...
  info "Hardening sshd"
  local cfg="/etc/ssh/sshd_config" bak="/etc/ssh/sshd_config.pre-n8n-bootstrap"
  [[ -f "$bak" ]] || cp "$cfg" "$bak"

  # enforce key-only, disable root login if requested
  declare -A opts=(
  ...
  # SAFE PORT MIGRATION — keep 22 + new port until you opt to close 22
  if ! grep -qE '^\s*Port\s+22\b' "$cfg"; then echo "Port 22" >> "$cfg"; fi
  if [[ "$SSH_PORT" -ne 22 ]]; then
    if grep -qE '^\s*Port\s+'"$SSH_PORT"'\b' "$cfg"; then :; else echo "Port $SSH_PORT" >> "$cfg"; fi
  fi
...
#-------------------------------------------------------------------------------------
# STEP 90: Stack files (n8n + Postgres + Caddy)
#-------------------------------------------------------------------------------------
step_90_stack() {
  ...
  # n8n encryption key (persisted)
  local N8N_ENCRYPTION_KEY_FILE="$N8N_DATA_DIR/.n8n_encryption_key"
  if [[ ! -s "$N8N_ENCRYPTION_KEY_FILE" ]]; then random_hex 32 > "$N8N_ENCRYPTION_KEY_FILE"; chmod 600 "$N8N_ENCRYPTION_KEY_FILE"; fi
...
step_110_summary() {
  ...
  info "Logs: cd $N8N_DATA_DIR && docker compose logs -f caddy n8n"
  mark_done "$step" "Summary printed"
}

confirm_before_reboot() {
  # Only prompt if interactive and confirmation requested
  if [[ "$AUTO_REBOOT" == "true" ]]; then return 0; fi
  if [[ "$CONFIRM_BEFORE_REBOOT" != "true" ]]; then return 0; fi
  if [[ ! -t 0 ]]; then warn "Non-interactive shell detected — skipping confirmation"; return 0; fi

  echo
  echo "A reboot is required to apply kernel/AppArmor updates."
  echo "Before rebooting, open a NEW SSH session in another terminal and verify login works:"
  echo
  echo "  ssh -p ${SSH_PORT} ${NEW_USER:-<your-user>}@${DOMAIN}"
  echo "  # or use server IP if DNS not ready"
  echo
  read -r -p "Type YES to confirm SSH works and reboot now (anything else to cancel): " ans
  if [[ "$ans" != "YES" ]]; then
    warn "Reboot canceled by user. Re-run this script later to continue."
    exit 0
  fi
}
...
main() {
  ...
  if [[ -f "$STATE_DIR/reboot_required" ]]; then
    warn "Rebooting to apply kernel/AppArmor changes is pending."
    confirm_before_reboot
